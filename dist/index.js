...
async function initWorkspace(cwd, parsed = { positionals: [], flags: new Map() }) {
    ...
    await writeIfMissing(path.join(devrtPath, "devrt.json"), JSON.stringify({ version: 1, runtime: "devrt", createdBy: "devrt init" }, null, 2) + "\n", cwd, created, preserved);
    await writeIfMissing(path.join(devrtPath, "manifest.json"), JSON.stringify({ version: 1, tools: [], actions: [], scenarios: [], verify: [] }, null, 2) + "\n", cwd, created, preserved);
...
async function installAgentFiles(cwd) {
...
async function upsertManagedBlock(filePath, blockContent) {
...
async function writeIfMissing(filePath, content, cwd, created, preserved) {
    if (existsSync(filePath)) {
...
async function createTask(cwd, parsed, stdin) {
    ...
    await writeFile(path.join(taskDir, "acceptance.json"), JSON.stringify({
        version: 1,
        taskId,
        derived: true,
        source: "task.md",
...
async function showTask(cwd, taskId) {
    ...
    const acceptance = await readJsonFile(path.join(taskDir, "acceptance.json"));
    return {
        ok: true,
        taskId: id,
        task,
...
async function listActions(cwd) {
...
async function listScenarios(cwd) {
    const manifest = await loadManifest(cwd);
...
async function validateActions(cwd) {
...
async function doctorWorkspace(cwd, parsed, env) {
    await ensureWorkspace(cwd);
    const manifest = await loadManifest(cwd);
...
async function runAction(cwd, parsed, env) {
    await ensureWorkspace(cwd);
...
async function executeAction(cwd, manifest, actionName, input, taskId, env) {
...
async function verifyTask(cwd, parsed, env) {
    await ensureWorkspace(cwd);
...
async function verifyScenarioByName(cwd, scenarioName, parsed, env) {
    await ensureWorkspace(cwd);
    const taskId = requireTaskFlag(parsed);
    await assertDirectory(getTaskDir(cwd, taskId), `Task not found: ${taskId}`);
    const manifest = await loadManifest(cwd);
...
async function verifyAllScenarios(cwd, manifest, taskId, env) {
    const startedAt = new Date();
...
async function verifyScenario(cwd, manifest, scenarioName, taskId, env) {
...
async function runScenario(cwd, manifest, scenario, taskId, env) {
...
async function runCleanupSteps(cwd, manifest, cleanupSteps, taskId, env, context) {
    const cleanup = [];
...
async function getTaskStatus(cwd, parsed) {
    await ensureWorkspace(cwd);
    const taskId = requireTaskFlag(parsed);
    await assertDirectory(getTaskDir(cwd, taskId), `Task not found: ${taskId}`);
...
async function replayLast(cwd, parsed, env) {
    await ensureWorkspace(cwd);
    const taskId = getStringFlag(parsed, "task");
    const traces = await readTraces(cwd, taskId);
...
async function loadManifest(cwd) {
    await ensureWorkspace(cwd);
    return readJsonFile(path.join(cwd, DEVRT_DIR, "manifest.json"));
}
async function validateManifest(cwd) {
    let manifest;
    try {
...
function validateActionDefinition(cwd, action) {
    const errors = [];
    const warnings = [];
...
function validateVerifyDefinition(check) {
    const errors = [];
...
function validateScenarioDefinition(scenario, manifest) {
    const errors = [];
...
function validateScenarioStep(step, actionNames, label) {
    const errors = [];
...
function findManifestDuplicates(manifest) {
    const duplicates = [];
...
function findDuplicateValues(type, values) {
...
async function checkTools(cwd, manifest, env, runProbes) {
    const checks = [];
    for (const tool of manifest.tools ?? []) {
...
function checkActionCommands(cwd, manifest) {
...
function checkScenarios(manifest) {
...
function checkCommandTarget(cwd, command) {
...
function evaluateAssertions(value, assertions) {
...
function resolveTemplateValue(value, context) {
    if (typeof value === "string") {
        ...
        return value.replace(/\$\{([^}]+)\}/g, (_match, pathExpression) => {
...
function validateJsonSchema(value, schema, pathName) {
    if (!schema) {
        return [];
    }
    const errors = [];
...
function buildActionResult(action, input, commandResult, structured, ok) {
...
async function runShellCommand(rootCwd, command, options) {
...
async function writeTrace(cwd, trace) {
...
async function readTraces(cwd, taskId) {
    ...
    for (const entry of entries.filter((file) => file.endsWith(".json")).sort()) {
        const trace = await readJsonFile(path.join(tracesPath, entry));
...
function collectLogs(commandResult, structured) {
...
function createRuntimeEnv(baseEnv, additions) {
...
function requireTaskFlag(parsed) {
    const taskId = getStringFlag(parsed, "task");
    if (!taskId) {
...
function getTaskDir(cwd, taskId) {
...
function projectAgentBlock(fileName) {
